...
async function generateJson(req, file) {
  const json = JSON.stringify(await req);
  fs.writeFile(`./lib/data/${file}.json`, json, function(err, result) {
    if(err) console.log(`Unable to generate a JSON file for ${file}.`, err);
  });
}
 
generateJson(getTypes(), "types");
generateJson(getTopics(), "topics");
generateJson(getArticles(), "articles"); 