...
export async function getPosts() {
  try {
    const posts =  await sheets.spreadsheets.values.get({
      spreadsheetId: spreadsheetId,
      range: "Posts!A2:X",
    });
    const postsCols = posts.data.values;
    if (postsCols.length) {
      return postsCols.map((col) => ({
        id: Number(col[0]) || null,
        slug: col[1] || null,
        title: col[2] || null,
        intro: col[3] || null, 
        type: Number(col[4]) || null,
        topic: Number(col[6]) || null,
        likes: Number(col[8]) || null,
        views: Number(col[9]) || null,
        date: col[10] || null,
        time: Number(col[11]) || null,
      }));
    }
  } 
  catch (err) { console.log("Error occured while getting the blog posts", err); }
  return [];
}
...
export async function generateData() {
  async function generateJsonFiles(req, file) {
    ...
    generateJsonFiles(getPosts(), "posts")
  )
}