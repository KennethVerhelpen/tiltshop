...
export const HomeView = (props: NewHomeViewProps) => {
	...
	useEffect(() => {
		...
		if (lastPageItemInView) {
			// We set the index of the item that will remove extra items when visible in the view.
		...
		if (firstPageItemInView && !lastPageItemInView) {
			...
			// as the the markedIndex will always be a mutliple of the topics length.
			if (markedIndex > 0) {
			...
			// so we retrun null, otehrwise we'll get negative numbers.
  ...
  return (
    <>
			<S.Fading className={'p-0 layout-row layout-align-center-start overflow-hidden'}>
				<div className={'layout-column layout-align-start-start flex-none pt-32'}>
				...
				<S.MiddleGrid width={gridWidth} className={'layout-row layout-wrap layout-align-center-center flex-none'}>
					{gridItems.map((topic, index) => {
						return (
							<Fragment key={topic.id + index}>
								{ (index === headerIndex) &&
									<S.HeaderWrapper ref={headerWrapper} key={'header'} width={headerWidth} className={'layout-column layout-align-center-center mb-64'}>
										<S.HeaderContent position={headerParams.position} size={headerParams.size} blur={headerParams.blur} opacity={headerParams.opacity} className={'fixed layout-column layout-align-center-center'}>
											<Header	theme={'dark'} rotation={true} className={'mb-48'}/>
											<SearchAutocomplete/>
										</S.HeaderContent>
									</S.HeaderWrapper>
								}
									<div ref={gridItemRefs.current[index]} className={clsx({'cascade fade-in-bottom speed-5' : (index <= topics.length - 1)}, 'p-8 layout-column layout-align-center-center')}>
										<div ref={handleInViewRef(index, markedIndex, gridItems)} className="shadow-5-secondary-900"> 
											<Topic className={'flex layout-column layout-align-center-center'} topic={topic} index={index} />
										</div>
									</div>
							</Fragment>
				...
				<div className={'layout-column layout-align-start-start flex-none pt-128'}>
					{gridItems.slice(0, sideItemsNumber).map((topic, index) => (
						<div key={topic.id} className={clsx({'cascade fade-in-bottom speed-5' : (index <= defaultGridRows - 1)}, 'p-8 layout-column layout-align-center-center')}>
							<Topic className={'flex layout-column layout-align-center-center'} topic={topic} index={index} />
						</div>
